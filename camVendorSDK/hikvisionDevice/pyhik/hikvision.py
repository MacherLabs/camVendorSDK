...
class HikCamera(object):
    ...
    def __init__(self, host=None, port=DEFAULT_PORT,
                 usr=None, pwd=None, callBack=None,verify_ssl=True):
        """Initialize device."""
    ...
    def alert_stream(self, reset_event, kill_event):
        ...
        while True:
            ...
            try:
                ...
                time.sleep(5)
                self.update_stale()
                #self.publish_changes('DVR offline',0)
                self.callBack(-1,"status",False)
                time.sleep(fail_count * 30)
                continue
    ...
    def process_stream(self, tree):
        ...
        if len(etype) > 0:
            ...
            if state:
                ...
                if estate != old_state:
                    self.publish_changes(etype, echid,estate)
                self.watchdog.pet()
                ...
                for eprop in echannels:
                    if eprop[3] is not None:
                        ...
                        if sec_elap > 5 and eprop[0] is True:
                            ...
                            self.publish_changes(etype, eprop[1],False)

    def publish_changes(self, etype, echid,state):
        ...
        self.callBack(echid,etype,state)
